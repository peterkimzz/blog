...
`Rollup.js`는 자바스크립트 번들러입니다. webpack이나 parcel이랑 비슷하다고 생각하시면 됩니다. 다수의 파일들을 하나의 파일로 묶어서 성능이나 재사용성 부분에서 이득을 가져올 수 있는거죠.

타 번들러와의 차별점이라면 `CommonJS` 방식을 사용하지 않고 표준화된 자바스크립트 ES6를 사용한다고 하지만, 사용해본 결과 딱히 무슨 차이점이 있는지는 잘 모르겠습니다.

어쨌든 롤업 또한 웹팩처럼 커맨드라인 인터페이스를 같이 제공해주기 때문에 `global` 옵션을 줘서 로컬 컴퓨터에 전역으로 CLI를 설치할 수는 있지만, 이번 포스팅에선 npm 패키지를 이용해 프로젝트 내에서만 사용하도록 해보겠습니다. -->
