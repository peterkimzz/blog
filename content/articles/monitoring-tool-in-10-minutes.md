...
이 모니터링 행위를 자동화 한다면, 웹사이트 혹은 API 서버의 상태가 정상적인지 주기적으로 파악하려면 계속해서 서비스에 HTTP 요청을 해보면 됩니다. 그리고 오류가 발생했을 땐 어딘가로 알림을 보내는거죠.
...
우리는 오로지 `Upptime.js`을 정상적으로 작동시키기 위한 `Upptime.js`전용 토큰을 발행할겁니다. 불필요한 권한을 주지 않는 것도 보안을 깅화하는 방법입니다.
...
설명은 뭐 장황하게 많았지만, 실제로 설정해보면 10분도 안되는 시간에 정말 쓸만한 모니터링 도구를 만들 수 있습니다.
