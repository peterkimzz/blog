...
const index_js = require('mdast-util-to-markdown/lib/handle/index.js');
...
const toMarkdown = {
  ...
  handlers: {
    ...
    image: (node, _, context, safeOptions) => {
      return index_js.handle.image(node, _, context, safeOptions) + attributes$4(node, context);
    },
    link: (node, _, context, safeOptions) => {
      return index_js.handle.link(node, _, context, safeOptions) + attributes$4(node, context);
    },
    strong: (node, _, context, safeOptions) => {
      return index_js.handle.strong(node, _, context, safeOptions) + attributes$4(node, context);
    },
    inlineCode: (node, _, context) => {
      return index_js.handle.inlineCode(node, _, context) + attributes$4(node, context);
    },
    emphasis: (node, _, context, safeOptions) => {
      return index_js.handle.emphasis(node, _, context, safeOptions) + attributes$4(node, context);
    }
  }
};
...
function createLabel(effects, ok, nok, type, markerType, stringType, disallowEol) {
  ...
  function label(code) {
    if (code === Codes.EOF || micromarkUtilCharacter.markdownLineEnding(code) || size > 999) {
