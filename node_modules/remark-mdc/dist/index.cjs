...
const micromarkFactoryWhitespace = require('micromark-factory-whitespace');
const micromarkCoreCommonmark = require('micromark-core-commonmark');
...
function stringifyFrontMatter(data, content = "") {
  data = flat__default.flatten(data, {
  ...
  return [
    ...
    yaml__default.dump(data, { lineWidth: -1 }).trim(),
    FRONTMATTER_DELIMITER,
    content
  ].join("\n");
}
function parseFrontMatter(content) {
  ...
  if (content.startsWith(FRONTMATTER_DELIMITER)) {
    ...
    if (idx !== -1) {
      ...
      if (frontmatter) {
        data = yaml__default.load(frontmatter);
  ...
  return {
    ...
    data: flat__default.unflatten(data || {}, {})
  };
}
...
const toMarkdown = {
  ...
  handlers: {
    ...
    image: (node, _, state, info) => {
      return mdastUtilToMarkdown.defaultHandlers.image(node, _, state, info) + attributes$4(node, state);
    },
    link: (node, _, state, info) => {
      return mdastUtilToMarkdown.defaultHandlers.link(node, _, state, info) + attributes$4(node, state);
    },
    strong: (node, _, state, info) => {
      return mdastUtilToMarkdown.defaultHandlers.strong(node, _, state, info) + attributes$4(node, state);
    },
    inlineCode: (node, _, state) => {
      state.compilePattern = state.compilePattern || compilePattern;
      return mdastUtilToMarkdown.defaultHandlers.inlineCode(node, _, state) + attributes$4(node, state);
    },
    emphasis: (node, _, state, info) => {
      return mdastUtilToMarkdown.defaultHandlers.emphasis(node, _, state, info) + attributes$4(node, state);
    }
  }
};
...
function attributes$4(node, context) {
  ...
  function quoted(key2, value2) {
    return key2 + "=" + quote + stringifyEntities.stringifyEntitiesLight(value2, { subset }) + quote;
  }
}
...
function exitAttributeIdValue(token) {
  this.getData("componentAttributes").push(["id", parseEntities.parseEntities(this.sliceSerialize(token))]);
}
function exitAttributeClassValue(token) {
  this.getData("componentAttributes").push(["class", parseEntities.parseEntities(this.sliceSerialize(token))]);
}
function exitAttributeValue(token) {
  const attributes = this.getData("componentAttributes");
  attributes[attributes.length - 1][1] = parseEntities.parseEntities(this.sliceSerialize(token));
}
...
function exitAttributes() {
  ...
  while (++index < attributes.length) {
    ...
    const name = scule.kebabCase(attribute[0]);
...
function createLabel(effects, ok, nok, type, markerType, stringType, disallowEol) {
  ...
  function label(code) {
    if (code === Codes.EOF || micromarkUtilCharacter.markdownLineEnding(code) || /* <https://github.com/micromark/micromark/blob/bf53bf9/lib/constant/constants.js#L34> */
    size > 999) {
...
function createAttributes(effects, ok, nok, attributesType, attributesMarkerType, attributeType, attributeIdType, attributeClassType, attributeNameType, attributeInitializerType, attributeValueLiteralType, attributeValueType, attributeValueMarker, attributeValueData, disallowEol) {
  ...
  function between(code) {
    ...
    if (code === Codes.colon || code === Codes.underscore || micromarkUtilCharacter.asciiAlpha(code)) {
      effects.enter(attributeType);
      effects.enter(attributeNameType);
      effects.consume(code);
      return code === Codes.colon ? bindAttributeName : name;
    }
    if (disallowEol && micromarkUtilCharacter.markdownSpace(code)) {
      return micromarkFactorySpace.factorySpace(effects, between, "whitespace")(code);
    }
    if (!disallowEol && micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
      return micromarkFactoryWhitespace.factoryWhitespace(effects, between)(code);
    }
    return end(code);
  }
  ...
  function shortcutStartAfter(code) {
    if (code === Codes.EOF || code === Codes.quotationMark || code === Codes.hash || code === Codes.apostrophe || code === Codes.dot || code === Codes.LessThan || code === Codes.equals || code === Codes.greaterThan || code === Codes.backTick || code === Codes.closingCurlyBracket || micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
      return nok(code);
    }
  ...
  function shortcut(code) {
    ...
    if (code === Codes.hash || code === Codes.dot || code === Codes.closingCurlyBracket || micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
  ...
  function bindAttributeName(code) {
    if (code === Codes.dash || micromarkUtilCharacter.asciiAlphanumeric(code)) {
      effects.consume(code);
      return bindAttributeName;
    }
    effects.exit(attributeNameType);
    if (disallowEol && micromarkUtilCharacter.markdownSpace(code)) {
      return micromarkFactorySpace.factorySpace(effects, bindAttributeNameAfter, "whitespace")(code);
    }
    if (!disallowEol && micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
      return micromarkFactoryWhitespace.factoryWhitespace(effects, bindAttributeNameAfter)(code);
    }
  ...
  function name(code) {
    if (code === Codes.dash || code === Codes.dot || code === Codes.colon || code === Codes.underscore || micromarkUtilCharacter.asciiAlphanumeric(code)) {
      effects.consume(code);
      return name;
    }
    effects.exit(attributeNameType);
    if (disallowEol && micromarkUtilCharacter.markdownSpace(code)) {
      return micromarkFactorySpace.factorySpace(effects, nameAfter, "whitespace")(code);
    }
    if (!disallowEol && micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
      return micromarkFactoryWhitespace.factoryWhitespace(effects, nameAfter)(code);
    }
  ...
  function valueBefore(code) {
    if (code === Codes.EOF || code === Codes.LessThan || code === Codes.equals || code === Codes.greaterThan || code === Codes.backTick || code === Codes.closingCurlyBracket || disallowEol && micromarkUtilCharacter.markdownLineEnding(code)) {
      return nok(code);
    }
    ...
    if (disallowEol && micromarkUtilCharacter.markdownSpace(code)) {
      return micromarkFactorySpace.factorySpace(effects, valueBefore, "whitespace")(code);
    }
    if (!disallowEol && micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
      return micromarkFactoryWhitespace.factoryWhitespace(effects, valueBefore)(code);
    }
  ...
  function valueUnquoted(code) {
    ...
    if (code === Codes.closingCurlyBracket || micromarkUtilCharacter.markdownLineEndingOrSpace(code)) {
  ...
  function valueQuotedBetween(code) {
    ...
    if (micromarkUtilCharacter.markdownLineEnding(code)) {
      return disallowEol ? nok(code) : micromarkFactoryWhitespace.factoryWhitespace(effects, valueQuotedBetween)(code);
    }
    effects.enter(attributeValueData);
    effects.consume(code);
    return valueQuoted;
  ...
  function valueQuoted(code) {
    if (code === marker || code === Codes.EOF || micromarkUtilCharacter.markdownLineEnding(code)) {
      effects.exit(attributeValueData);
      return valueQuotedBetween(code);
    }
    effects.consume(code);
  ...
  function valueQuotedAfter(code) {
    return code === Codes.closingCurlyBracket || micromarkUtilCharacter.markdownLineEndingOrSpace(code) ? between(code) : end(code);
  }
  function end(code) {
...
function tokenize$5(effects, ok, nok) {
  ...
  function start(code) {
    ...
    if (micromarkUtilCharacter.markdownLineEnding(self.previous) || !event || !validEvents.includes(event[1].type)) {
      return nok(code);
    }
...
function createName(effects, ok, nok, nameType) {
  ...
  return start;
  function start(code) {
    if (micromarkUtilCharacter.asciiAlpha(code)) {
  ...
  function name(code) {
    if (code === Codes.dash || code === Codes.underscore || micromarkUtilCharacter.asciiAlphanumeric(code)) {
      effects.consume(code);
      return name;
    }
...
function tokenize$3(effects, ok, nok) {
  ...
  function start(code) {
    ...
    if (self.previous !== null && !micromarkUtilCharacter.markdownLineEndingOrSpace(self.previous) && ![Codes.openingSquareBracket, Codes.star, Codes.underscore].includes(self.previous)) {
      return nok(code);
    }
...
function tokenizeFrontMatter(effects, ok, _nok, next, initialPrefix) {
  ...
  function tokenizeDataSection(effects2, ok2, nok) {
    ...
    function closingSectionSequence(code) {
      if (code === Codes.dash || micromarkUtilCharacter.markdownSpace(code)) {
        effects2.consume(code);
        size++;
      ...
      return micromarkFactorySpace.factorySpace(effects2, ok2, "whitespace")(code);
    }
  }
  ...
  function dataSectionClose(code) {
    effects.exit("componentContainerDataSection");
    return micromarkFactorySpace.factorySpace(effects, next, "whitespace")(code);
  }
}
...
function tokenize$2(effects, ok, nok) {
  ...
  function sectionOpen(code) {
    ...
    if (micromarkUtilCharacter.markdownLineEnding(code)) {
      return micromarkFactorySpace.factorySpace(effects, lineStart, "whitespace")(code);
    }
  ...
  function sectionTitle(code) {
    if (micromarkUtilCharacter.markdownLineEnding(code)) {
      ...
      return micromarkFactorySpace.factorySpace(effects, lineStart, "linePrefix", 4)(code);
    }
    effects.consume(code);
    return sectionTitle;
  }
  ...
  function afterAttributes(code) {
    return micromarkFactorySpace.factorySpace(effects, openAfter, "whitespace")(code);
  }
  ...
  function contentStart(code) {
    ...
    if (containerFirstLine && (code === Codes.dash || micromarkUtilCharacter.markdownSpace(code))) {
  ...
  function lineStart(code) {
    ...
    return initialPrefix ? micromarkFactorySpace.factorySpace(effects, lineStartAfterPrefix, "linePrefix", initialPrefix + 1)(code) : lineStartAfterPrefix(code);
  }
  ...
  function tokenizeClosingFence(effects2, ok2, nok2) {
    let size = 0;
    return micromarkFactorySpace.factorySpace(effects2, closingPrefixAfter, "linePrefix", 4);
    function closingPrefixAfter(code) {
    ...
    function closingSequence(code) {
      ...
      return micromarkFactorySpace.factorySpace(effects2, closingSequenceEnd, "whitespace")(code);
    }
    function closingSequenceEnd(code) {
      if (code === null || micromarkUtilCharacter.markdownLineEnding(code)) {
...
function tokenize$1(effects, ok, nok) {
  const self = this;
  return micromarkFactorySpace.factorySpace(effects, lineStart, "linePrefix");
  function lineStart(code) {
    ...
    switch (code) {
      case Codes.backTick:
        return micromarkCoreCommonmark.codeFenced.tokenize.call(self, effects, ok, nok)(code);
      case Codes.colon:
...
function tokenize(effects, ok, nok) {
  ...
  const tokenizeSugerSyntax = tokenizeInline.tokenize.call(
    self,
    effects,
    micromarkFactorySpace.factorySpace(effects, exit, "linePrefix"),
    nok
  );
  return micromarkFactorySpace.factorySpace(effects, lineStart, "linePrefix");
  function lineStart(code) {
  ...
  function exit(code) {
    if (micromarkUtilCharacter.markdownLineEnding(code) || code === Codes.EOF) {
      return ok(code);
    }
    return nok(code);
  }
...
const index = (function({ components = [] } = {}) {
  ...
  if (components.length) {
    return async (tree, { data: data2 }) => {
      const jobs = [];
      unistUtilVisit.visit(tree, ["textComponent", "leafComponent", "containerComponent"], (node) => {
        bindNode(node);
