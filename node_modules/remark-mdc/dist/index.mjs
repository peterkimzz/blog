...
const toMarkdown = {
  ...
  handlers: {
    ...
    image: (node, _, state, info) => {
      return defaultHandlers.image(node, _, state, info) + attributes$4(node, state);
    },
    link: (node, _, state, info) => {
      return defaultHandlers.link(node, _, state, info) + attributes$4(node, state);
    },
    strong: (node, _, state, info) => {
      return defaultHandlers.strong(node, _, state, info) + attributes$4(node, state);
    },
    inlineCode: (node, _, state) => {
      state.compilePattern = state.compilePattern || compilePattern;
      return defaultHandlers.inlineCode(node, _, state) + attributes$4(node, state);
    },
    emphasis: (node, _, state, info) => {
      return defaultHandlers.emphasis(node, _, state, info) + attributes$4(node, state);
    }
  }
};
